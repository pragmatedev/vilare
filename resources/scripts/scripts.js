...
document.addEventListener('alpine:init', () => {
  window.Alpine.data('vilare', () => ({
    init() {
      if (!window.vilare.ajax) {
        window.vilare.ajax = ajax;
      }

      if (!window.vilare.scroll) {
        window.vilare.scroll = scroll;
      }
    },
  }));
});
...
document.addEventListener('DOMContentLoaded', () => {
  document.documentElement.style.setProperty('--container-full', `calc(${document.documentElement.clientWidth * 0.01}px * 100)`);
});
...
window.addEventListener('load', () => {
  document.dispatchEvent(new CustomEvent('vilare:init'));
});
